...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	...
	if serviceType == "meta" || serviceType == "both" {
		srvMeta := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterMetaStoreServer(grpcServer, srvMeta)
	}
	if serviceType == "block" || serviceType == "both" {
		srvBlock := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(grpcServer, srvBlock)
	}
