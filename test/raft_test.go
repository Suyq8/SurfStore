...
func TestRaftCrash(t *testing.T) {
	t.Log("leader1 gets a request")
	//Setup
	cfgPath := "./config_files/3nodes.txt"
	test := InitTest(cfgPath)
	...
	for idx, server := range test.Clients {
		...
		if idx != 2 {
			...
			log := make([]*surfstore.UpdateOperation, 0)
			_, err = CheckInternalState(&leader, &term, log, make(map[string]*surfstore.FileMetaData), server, test.Context)
		}
...
func TestRaftRecoverable(t *testing.T) {
	...
	// all logs and metastores should be empty
	// ^ TODO check
	for idx, client := range test.Clients {
		if idx != leaderIdx {
			client.Crash(test.Context, &emptypb.Empty{})
		}
	}
	...
	go func() {
		...
		for idx, client := range test.Clients {
			if idx != leaderIdx {
				client.Restore(test.Context, &emptypb.Empty{})
			}
		}
	}()
	test.Clients[leaderIdx].UpdateFile(test.Context, filemeta)
